...
**Implementation Files:**
- `src/modules/event-scheduling/adapters/persistence/PrismaEventRepository.ts` (claimReadyEvents method)
...
This pattern uses **PostgreSQL row-level locking** (`FOR UPDATE SKIP LOCKED`) to enable multiple scheduler instances to atomically claim events from a shared queue without duplicate processing.

**Key characteristics:**
- ✅ Zero duplicate processing (each event claimed by exactly one worker)
- ✅ Horizontal scalability (add more instances = more throughput)
- ✅ No deadlocks (SKIP LOCKED avoids waiting)
- ✅ Sub-millisecond lock acquisition
- ✅ Works with any number of concurrent scheduler instances
...
async claimReadyEvents(limit: number): Promise<Event[]> {
  return this.prisma.$transaction(async (tx) => {
    ...
    const events = await tx.$queryRaw<Array<RawEvent>>`
      ...
      LIMIT ${limit}
      FOR UPDATE SKIP LOCKED  -- Critical: Locks rows atomically
    `;
...
**How it works:**
1. Instance A locks events 1-3 with `FOR UPDATE`
2. Instance B skips locked rows with `SKIP LOCKED`, claims events 4-6
3. Instance C claims events 7-9
4. **Result:** All instances busy, no duplicates, no waiting
...
For detailed information about database locking strategies, including:
- Complete explanation of pessimistic vs optimistic locking
- Why Prisma's query builder can't handle this use case
- Transaction wrapper requirements
- Performance benchmarks and optimization strategies
- Index design and query performance
- Testing concurrency
- Use cases beyond this system

**See: [Database Locking Strategies](./database-locking.md)**
...
- Task schedulers distributing jobs across workers
- Ticket booking systems
- Inventory management with concurrent orders
- Email/SMS campaign delivery systems
