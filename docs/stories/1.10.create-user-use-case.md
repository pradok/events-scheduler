...
**Note:** This story was originally 1.8 and marked as Done. It has been renumbered to 1.10 and requires refactoring to implement bounded contexts with domain events. The current implementation creates Events directly (tight coupling); it needs to be refactored to publish UserCreated domain events instead.
...
**As a** developer,
**I want** a CreateUser use case that publishes UserCreated domain events,
**so that** User Context is decoupled from Event Scheduling Context via domain events.

---

## Acceptance Criteria

**UPDATED for Bounded Contexts Architecture:**

1. CreateUserUseCase refactored in `src/application/use-cases/user/CreateUserUseCase.ts`
2. Use case receives IUserRepository and IDomainEventBus via dependency injection (NO IEventRepository, NO TimezoneService, NO EventHandlerRegistry)
...
5. Use case validates input DTO using Zod schema and derived types ✅ Already done
6. Use case creates User domain entity ✅ Already done
7. Use case saves user to database via IUserRepository ✅ Already done
8. Use case publishes UserCreatedEvent to IDomainEventBus (NEW)
