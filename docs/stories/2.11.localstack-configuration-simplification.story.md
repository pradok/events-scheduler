...
- [ ] **Task 1: Audit Current LocalStack Configuration** (AC: 1, 2, 3)
  - [ ] Review `docker/localstack/init-aws.sh` and identify all resources created
  - [ ] Review `scripts/deploy-lambda.js` and identify queue naming patterns
  - [ ] Review integration tests (`SQSAdapter.integration.test.ts`, `schedulerHandler.integration.test.ts`) for queue creation
  - [ ] Document current queue naming inconsistencies (if any)
  - [ ] Create list of resources that should be in init script vs test-created
  - [ ] Reference: [Source: docker/localstack/init-aws.sh]
  - [ ] Reference: [Source: scripts/deploy-lambda.js:78-85]

- [ ] **Task 2: Consolidate Queue Naming to Single Standard** (AC: 1)
  - [ ] Choose single queue name: `events-queue` (matches init-aws.sh)
  - [ ] Update `scripts/deploy-lambda.js` to use `events-queue` instead of `bday-events-queue`
  - [ ] Verify init-aws.sh already creates `events-queue` and `events-dlq`
  - [ ] Document queue naming convention in architecture docs
  - [ ] Reference: [Source: docker/localstack/init-aws.sh:20-39]

- [ ] **Task 3: Update Integration Tests to Create Isolated Test Queues** (AC: 2, 4)
  - [ ] Modify `SQSAdapter.integration.test.ts` to continue creating unique test queues (no change needed - already correct)
  - [ ] Verify test uses `CreateQueueCommand` in `beforeEach` and `DeleteQueueCommand` in `afterEach`
  - [ ] Verify test queues use timestamp-based names for isolation: `test-queue-${Date.now()}`
  - [ ] Update `schedulerHandler.integration.test.ts` if it creates queues (verify pattern)
  - [ ] Document test isolation pattern in architecture docs
  - [ ] Reference: [Source: src/adapters/secondary/messaging/SQSAdapter.integration.test.ts:42-87]
  - [ ] Reference: [Source: docs/architecture/test-strategy.md#Integration-Tests]

- [ ] **Task 4: Simplify init-aws.sh to Static Infrastructure Only** (AC: 3)
  - [ ] Keep in init-aws.sh:
  ...
  - [ ] Remove from init-aws.sh (if present):
    - Lambda function deployment (handled by deploy-lambda.js)
    - EventBridge target configuration (handled by deploy-lambda.js)
  - [ ] Verify init-aws.sh focuses only on infrastructure that persists across Lambda deployments
  - [ ] Add comments explaining separation of concerns
  - [ ] Reference: [Source: docker/localstack/init-aws.sh]

- [ ] **Task 5: Verify deploy-lambda.js Handles Dynamic Resources** (AC: 4, 6)
  - [ ] Ensure deploy-lambda.js handles:
    - Lambda function creation/deletion
    - EventBridge target configuration
    - Lambda permissions for EventBridge
  - [ ] Update queue name from `bday-events-queue` to `events-queue` for consistency
  - [ ] Verify script uses AWS SDK v3 clients (already correct)
  - [ ] Add error handling for queue not found (should fail fast if init-aws.sh didn't run)
  - [ ] Reference: [Source: scripts/deploy-lambda.js:61-194]

- [ ] **Task 6: Remove Deprecated Shell Scripts** (AC: 5)
  - [ ] Identify deprecated scripts to remove:
    - `scripts/lambda-deploy.sh` (replaced by deploy-lambda.js)
    - `scripts/lambda-eventbridge.sh` (functionality in deploy-lambda.js)
  - [ ] Verify no npm scripts reference these files
  - [ ] Delete deprecated scripts using `git rm`
  - [ ] Update package.json if any scripts referenced the deleted files
  - [ ] Reference: [Source: scripts/lambda-deploy.sh]
  - [ ] Reference: [Source: scripts/lambda-eventbridge.sh]

- [ ] **Task 7: Update Documentation - Architecture** (AC: 7, 9)
  - [ ] Create or update `docs/architecture/localstack-setup.md` with:
  ...
  - [ ] Add section on static vs dynamic resources
  - [ ] Include diagram showing resource creation flow
  - [ ] Document why we separate static infrastructure from Lambda deployment
  - [ ] Reference: [Source: docs/architecture/tech-stack.md#Local-AWS]

- [ ] **Task 8: Add Troubleshooting Guide** (AC: 10)
  - [ ] Create `docs/architecture/localstack-troubleshooting.md` with:
  ...
  - [ ] Add debugging tips for common LocalStack issues
  - [ ] Include links to LocalStack documentation

- [ ] **Task 9: Update README or Developer Documentation** (AC: 7)
  ...
  - [ ] Add troubleshooting section linking to localstack-troubleshooting.md
  - [ ] Remove references to deprecated scripts
  - [ ] Reference: [Source: docs/architecture/localstack-setup.md]

- [ ] **Task 10: Verify All Tests Pass** (AC: 8)
  - [ ] Run full test suite: `npm test`
  - [ ] Specifically verify integration tests:
  ...
  - [ ] Verify tests create and clean up their own resources
  - [ ] Verify no test failures due to queue naming changes
