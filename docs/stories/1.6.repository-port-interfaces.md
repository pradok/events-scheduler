...
- [ ] **Task 1: Create application/ports directory structure** (AC: 1, 2)
  - [ ] Create `src/application/` directory if it doesn't exist
  - [ ] Create `src/application/ports/` subdirectory
  - [ ] Verify directory structure matches architecture source tree

- [ ] **Task 2 (TDD): Define IUserRepository interface** (AC: 1, 3, 5, 6, 7, 9)
  - [ ] **RED: Write failing test first**
    - [ ] Create `src/application/ports/IUserRepository.test.ts`
    - [ ] Write test: "should define interface with required methods (create, findById, findByEmail, findUsersWithUpcomingBirthdays, update, delete)"
    - [ ] Write test: "should use User domain entity not Prisma models"
    - [ ] Write test: "should have explicit return type annotations"
    - [ ] Run tests → expect failures (interface doesn't exist yet)
  - [ ] **GREEN: Implement IUserRepository to pass tests**
    - [ ] Create `src/application/ports/IUserRepository.ts` file
    - [ ] Import User entity from domain layer (`src/domain/entities/User`)
    - [ ] Define IUserRepository interface with PascalCase naming and `I` prefix
    - [ ] Define `create(user: User): Promise<User>` method with JSDoc comment
    - [ ] Define `findById(userId: string): Promise<User | null>` method with JSDoc comment
    - [ ] Define `findByEmail(email: string): Promise<User | null>` method with JSDoc comment explaining uniqueness constraint
    - [ ] Define `findUsersWithUpcomingBirthdays(daysAhead: number): Promise<User[]>` method with JSDoc comment explaining batch processing use case
    - [ ] Define `update(user: User): Promise<User>` method with JSDoc comment
    - [ ] Define `delete(userId: string): Promise<void>` method with JSDoc comment explaining user deletion
    - [ ] Add comprehensive JSDoc comments explaining each method's purpose, parameters, and return values
    - [ ] Export interface as named export
    - [ ] Run tests → expect all tests pass
  - [ ] **REFACTOR: Clean up and verify**
    - [ ] Verify zero imports from Prisma, adapters, or infrastructure
    - [ ] Verify TypeScript strict mode compilation passes
    - [ ] Review JSDoc for completeness and clarity
    - [ ] Run tests again → expect all tests still pass

- [ ] **Task 3 (TDD): Define IEventRepository interface** (AC: 2, 4, 5, 6, 7, 8, 9)
  - [ ] **RED: Write failing test first**
    - [ ] Create `src/application/ports/IEventRepository.test.ts`
    - [ ] Write test: "should define interface with required methods (create, findById, findByUserId, update, claimReadyEvents)"
    - [ ] Write test: "should use Event domain entity not Prisma models"
    - [ ] Write test: "should have explicit return type annotations"
    - [ ] Run tests → expect failures (interface doesn't exist yet)
  - [ ] **GREEN: Implement IEventRepository to pass tests**
    - [ ] Create `src/application/ports/IEventRepository.ts` file
    - [ ] Import Event entity from domain layer (`src/domain/entities/Event`)
    - [ ] Define IEventRepository interface with PascalCase naming and `I` prefix
    - [ ] Define `create(event: Event): Promise<Event>` method with JSDoc comment
    - [ ] Define `findById(eventId: string): Promise<Event | null>` method with JSDoc comment
    - [ ] Define `findByUserId(userId: string): Promise<Event[]>` method with JSDoc comment explaining querying user's events
    - [ ] Define `update(event: Event): Promise<Event>` method with JSDoc comment
    - [ ] Define `claimReadyEvents(limit: number): Promise<Event[]>` method with detailed JSDoc explaining: (1) Dual purpose: normal polling every minute + recovery of missed events after system downtime, (2) Atomic claim operation with locking to prevent race conditions, (3) Returns events where targetTimestampUTC <= now and status = PENDING, (4) Optimistic locking via version field
    - [ ] Export interface as named export
    - [ ] Run tests → expect all tests pass
  - [ ] **REFACTOR: Clean up and verify**
    - [ ] Verify zero imports from Prisma, adapters, or infrastructure
    - [ ] Verify TypeScript strict mode compilation passes
    - [ ] Review JSDoc for completeness and clarity, especially claimReadyEvents dual purpose explanation
    - [ ] Run tests again → expect all tests still pass

- [ ] **Task 4: Final validation and compilation** (AC: 5, 9)
  - [ ] Run complete test suite: `npm test`
  - [ ] Verify all tests pass (IUserRepository + IEventRepository tests)
  - [ ] Run `npm run build` to verify TypeScript compilation
  - [ ] Verify no `any` types used in interface definitions
  - [ ] Verify all method signatures have explicit return type annotations
  - [ ] Verify test coverage is 100% for both port interface files
  - [ ] Verify claimReadyEvents JSDoc clearly explains dual purpose (normal polling + recovery)
