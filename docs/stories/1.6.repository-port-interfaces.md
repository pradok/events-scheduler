...
- [ ] **Task 2 (TDD): Define IUserRepository interface** (AC: 1, 3, 5, 6, 7, 8)
  - [ ] **RED: Write failing test first**
    - [ ] Create `src/application/ports/IUserRepository.test.ts`
    - [ ] Write test: "should define interface with required methods (create, findById, update, delete)"
  ...
  - [ ] **GREEN: Implement IUserRepository to pass tests**
    ...
    - [ ] Define `delete(userId: string): Promise<void>` method with JSDoc comment
...
- [ ] **Task 3 (TDD): Define IEventRepository interface** (AC: 2, 4, 5, 6, 7, 8)
  - [ ] **RED: Write failing test first**
    - [ ] Create `src/application/ports/IEventRepository.test.ts`
    - [ ] Write test: "should define interface with required methods (create, findById, update, findReadyEvents, findMissedEvents)"
  ...
  - [ ] **GREEN: Implement IEventRepository to pass tests**
    ...
    - [ ] Define `findReadyEvents(limit: number): Promise<Event[]>` method with detailed JSDoc explaining atomicity requirements
    - [ ] Define `findMissedEvents(limit: number): Promise<Event[]>` method with JSDoc comment
    - [ ] Add JSDoc comments explaining optimistic locking and race condition protection requirements
    - [ ] Export interface as named export
    - [ ] Run tests → expect all tests pass
  - [ ] **REFACTOR: Clean up and verify**
    - [ ] Verify zero imports from Prisma, adapters, or infrastructure
...
- [ ] **Task 4: Final validation and compilation** (AC: 5, 8)
...
- Cascade delete behavior documented in JSDoc but not in interface signature
...
interface IEventRepository {
  ...
  // Find missed events (targetTimestampUTC in past, still PENDING)
...
- `findReadyEvents()` MUST be atomic (prevent race conditions)
